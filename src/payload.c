...
static const u8 kernel_magic[] = {0x00, 0x00, 0xa0, 0xe1, 0x00, 0x00, 0xa0, 0xe1}; // bunch of nops
...
{
    UNUSED(size);
    unsigned int source_len = 0, dest_len = 1 << 26; // 64 MB should be enough hopefully
...
{
    UNUSED(size);
    uint32_t source_len = 0, dest_len = 1 << 26; // 64 MB should be enough hopefully
...
static void *load_kernel(void *p, size_t size)
{
    ...
    if (((uintptr_t)kernel) & (KERNEL_ALIGN - 1)) {
        void *new_addr = heapblock_alloc_aligned(size, KERNEL_ALIGN);
        memcpy(new_addr, kernel, size);
        kernel = new_addr;
    }

    // big hack, 20 MB, for kernel decompressor too
    return (void *)((uintptr_t)kernel + 0x1400000);
}
...
static void *load_one_payload(void *start, size_t size)
{
    ...
    } else if (!memcmp(p, kernel_magic, sizeof kernel_magic)) {
...
int payload_run(void *_payload_start, void *_payload_end)
{
    const char *target = adt_getprop(adt, 0, "compatible", NULL);
    if (target) {
    ...
    while (p && p < _payload_end)
        p = load_one_payload(p, _payload_end - _payload_start);
